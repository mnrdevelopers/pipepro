...
async function saveProject() {
    ...
    try {
        ...
        if (currentProjectId) {
            ...
            if (customerId) {
                const custDoc = await db.collection('users').doc(businessId).collection('customers').doc(customerId).get();
                if (custDoc.exists) {
...
window.editProject = async (id) => {
    ...
    try {
        ...
        if (projectCustomerSelect) {
            projectCustomerSelect.value = order.customerId || '';
            if (!projectCustomerSelect.value && order.customerName) {
                const match = Array.from(projectCustomerSelect.options).find((o) =>
                    (o.textContent || '').toLowerCase().startsWith(order.customerName.toLowerCase())
                );
                if (match) projectCustomerSelect.value = match.value;
            }
        }
...
async function checkPendingActions() {
    ...
    if (openModal) {
        ...
        resetProjectForm();
        if (projectCustomerSelect && projectCustomerSelect.options.length <= 1) {
            await ensureProjectCustomersLoaded();
        }
        ...
        if (customerData) {
            ...
            if (projectCustomerSelect) {
                if (customer.id) projectCustomerSelect.value = customer.id;
                if (!projectCustomerSelect.value && customer.name) {
                    const match = Array.from(projectCustomerSelect.options).find((o) =>
                        (o.textContent || '').toLowerCase().startsWith(customer.name.toLowerCase())
                    );
                    if (match) projectCustomerSelect.value = match.value;
                }
            }
