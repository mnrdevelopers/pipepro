...
function setupGstListeners() {
    ...
    if (gstCustomer) {
        gstCustomer.addEventListener('change', () => {
            ...
            if (gstPlaceOfSupply && option?.dataset?.state && !gstPlaceOfSupply.value) {
                gstPlaceOfSupply.value = option.dataset.state;
            }
            updateGstPreview();
        });
    }
    ...
    if (gstPlaceOfSupply) gstPlaceOfSupply.addEventListener('input', updateGstPreview);
...
async function loadGstDocs() {
    ...
    const snapshot = await db.collection('users').doc(businessId).collection('gstDocuments').orderBy('createdAt', 'desc').get();
    gstDocs = [];
    snapshot.forEach(doc => gstDocs.push({ id: doc.id, ...doc.data() }));
