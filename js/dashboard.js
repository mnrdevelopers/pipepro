...
async function handleHashChange() {
    ...
    const activeLink = document.querySelector(`.sidebar .nav-link[data-section="${hash}"]`);
    ...
    // Trigger data load
    window.dispatchEvent(new CustomEvent('sectionChanged', { detail: hash }));
...
async function loadDashboardData() {
    ...
    try {
        ...
        const transactionsSnapshot = await db.collection('users').doc(businessId)
                ...
                transactionsSnapshot.forEach(doc => {
                    ...
                    const row = `
                        <tr>
                            ...
                            <td>
                                ${statusBadge}
                                ${canDelete ? `<button class="btn btn-sm btn-link text-danger ms-2 p-0" onclick="window.deleteTransaction('${doc.id}')" title="Delete"><i class="fas fa-times"></i></button>` : ''}
                            </td>
                        </tr>
                    `;
...
function setupEventListeners() {
    ...
    const confirmDeleteBtn = document.getElementById('confirmDeleteTransactionBtn');
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', async () => {
            ...
            try {
                await db.collection('users').doc(businessId).collection('transactions').doc(transactionToDeleteId).delete();
...
function setupNotifications(businessId) {
    ...
    db.collection('users').doc(businessId).collection('inventory')
        .onSnapshot(snapshot => {
            const lowStockItems = [];
            snapshot.forEach(doc => {
                const item = doc.data();
    ...
    db.collection('users').doc(businessId).collection('vehicles')
        .onSnapshot(snapshot => {
            const alerts = [];
...
export { checkAuth, loadUserData, formatDate, getStatusClass };