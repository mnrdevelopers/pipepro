...
async function openInvoiceModal() {
    ...
    try {
        const [custSnap, invSnap, vehicleSnap, projectSnap] = await Promise.all([
            ...
            db.collection('users').doc(businessId).collection('inventory').where('category', 'in', ['Finished Goods', 'PVC Pipes', 'Fittings']).get(),
...
function getInvoicesExportRows() {
    return invoicesData.map(inv => ([
        formatDate(inv.date),
        `#${String(inv.id || '').substr(0, 6).toUpperCase()}`,
        inv.customer || '',
        inv.amount ?? 0,
        inv.status || ''
    ]));
...
async function saveInvoice() {
    ...
    try {
        ...
        // Verify stock & create invoice in one transaction
        await db.runTransaction(async (transaction) => {
            for (const item of items) {
                ...
                if (currentQty < item.quantity) {
                    throw new Error(`Insufficient stock for ${itemSnap.data().name}. Available: ${currentQty}`);
                }
                transaction.update(itemRef, { quantity: currentQty - item.quantity });
            }
