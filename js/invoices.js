...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    loadInvoices();
    setupEventListeners();
    if (!invoiceFormReady) openInvoiceModal({ scroll: false });
    
    window.addEventListener('sectionChanged', (e) => {
        if (e.detail === 'invoices') {
            loadInvoices();
            if (!invoiceFormReady) openInvoiceModal();
        }
    });
});
...
async function openInvoiceModal(options = {}) {
    ...
    try {
        const [custSnap, invSnap, vehicleSnap, projectSnap] = await Promise.all([
            ...
            db.collection('users').doc(businessId).collection('inventory').where('category', 'in', ['RCC Pipes', 'Septic Tank Products']).get(),
        ...
        if (pendingOrderPrefill) {
            ...
            if (order.name) {
                projectSelect.value = order.name;
            }
...
function toggleShipToFields() {
    ...
    const fields = [
        'invShipName', 'invShipPhone', 'invShipAddress', 'invShipCity',
...
function syncShipToFromBillTo() {
    ...
    setVal('invShipCity', opt.dataset.city || '');
...
function getShipToData() {
    ...
    return {
        ...
        shipCity: document.getElementById('invShipCity')?.value || '',
...
async function fillShipToFromPincode() {
    ...
    const cityEl = document.getElementById('invShipCity');
...
function getInvoiceTemplate(type, data) {
    ...
    const companyName = company.companyName || 'PipePro Business';
