...
async function handleLogin(e) {
    ...
    try {
        ...
        if (ownerGateEnabled && onLogin && sessionStorage.getItem('ownerGate') !== 'ok') {
...
async function handleRegister(e) {
    ...
    try {
        ...
        const ownerGateEnabled = await isOwnerGateEnabled(user.uid);
        const onLogin = window.location.pathname.includes('login.html');
        const urlMode = new URLSearchParams(window.location.search).get('mode');
        if (ownerGateEnabled && onLogin && sessionStorage.getItem('ownerGate') !== 'ok') {
            sessionStorage.removeItem('ownerGate');
    ...
    if (user) {
        ...
        if (currentPath.includes('login.html')) {
            ...
            isOwnerGateEnabled(user.uid).then((enabled) => {
                if (enabled) {
                    if (sessionStorage.getItem('ownerGate') === 'ok') {
                        window.location.href = 'dashboard.html';
                    }
                } else {
                    window.location.href = 'dashboard.html';
        ...
        if (currentPath.includes('register.html') || currentPath === '/' || currentPath.endsWith('/index.html')) {
            window.location.href = 'dashboard.html';
        }
    } else {
