        ...
        <tr data-id="${item.id}">
            ...
            <td>
                <div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-primary edit-item" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-outline-info view-history" title="History">
                        <i class="fas fa-history"></i>
                    </button>
                    <button class="btn btn-outline-success stock-in" title="Stock In">
                        <i class="fas fa-arrow-down"></i>
                    </button>
                    <button class="btn btn-outline-warning stock-out" title="Stock Out">
...
function getItemIcon(category) {
    const iconMap = {
        ...
        'Steel': 'bars',
        'Additives': 'flask',
        'Finished Goods': 'box-open',
        'Molds': 'shapes',
        'PVC Pipes': 'pipe',
        'Septic Tanks': 'dumpster',
        'Fittings': 'cogs',
        'Tools': 'tools',
        'Equipment': 'toolbox',
...
function setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
        ...
        } else if (e.target.closest('.stock-in')) {
            showStockModal(item, 'in');
        } else if (e.target.closest('.stock-out')) {
            showStockModal(item, 'out');
        }
    });
}
...
function exportInventoryCSV() {
    ...
    const headers = ['Item', 'Category', 'Description', 'Qty', 'Reorder', 'Unit', 'Cost', 'Price'];
...
async function showAddModal() {
    ...
    // Set default values
    document.getElementById('category').value = 'Finished Goods';
    document.getElementById('unit').value = 'piece';
    document.getElementById('reorderLevel').value = 10;
    
    await populateSupplierDropdown('supplier');
...
async function showStockModal(item, type) {
    ...
    if (type === 'in') {
        ...
        document.getElementById('stockInReason').value = 'Purchase';
        document.getElementById('stockInCost').value = item.costPrice || '';
        document.getElementById('stockInGst').value = '18';
        document.getElementById('stockInFile').value = '';
...
async function handleStockIn() {
    ...
    const user = JSON.parse(localStorage.getItem('user'));
    const businessId = user.businessId || user.uid;
    const qty = parseInt(document.getElementById('stockInQuantity').value);
    ...
    const gstPercent = parseFloat(document.getElementById('stockInGst').value) || 0;
    const totalCost = (qty * cost) + ((qty * cost) * (gstPercent / 100)) + transportCost;
    const fileInput = document.getElementById('stockInFile');
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            transaction.set(purchaseRef, {
                ...
                invoiceFile: fileInput && fileInput.files.length > 0 ? fileInput.files[0].name : null
            });
        });
...
// Export functions
export { loadInventory, showAddModal, showEditModal };
